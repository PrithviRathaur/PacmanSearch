...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        visitedCorners = ();
        # check if starting position is in corners
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.EAST, Directions.WEST, Directions.NORTH, Directions.SOUTH]:
            ...
            newPos = nextx, nexty
            if not hitsWall:
                if newPos in self.corners and newPos not in visitedCorners:
                    visitedCorners = visitedCorners + (newPos,)
                    newState = (newPos, visitedCorners)
                else:
                    newState = (newPos, visitedCorners)
...
def cornersHeuristic(state, problem):
    ...
    while notVisited:
        ...
        currentPos = notVisited.pop(index)
...
def foodHeuristic(state, problem):
    ...
    for firstFood in foodList:
        for secondFood in foodList:
            distance = util.manhattanDistance(firstFood, secondFood)
            if distance >= heuristic:
                distanceToFirst = util.manhattanDistance(position, firstFood)
                distanceToSecond = util.manhattanDistance(position, secondFood)
                if distanceToFirst > distanceToSecond:
                    heuristic = distance + distanceToSecond
                else:
                    heuristic = distance + distanceToFirst
    return heuristic
